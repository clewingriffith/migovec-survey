...
"""
Create a survey using move and line commands
"""
class SurveyWriter:
	def __init__(self):
		self.strips = []
	...
	def move(self,x,y,z):
		print "SurveyWriter.move"
		self.strips.append([(x,y,z)])
		
	def line(self,x,y,z):
		currentStrip = self.strips[-1]
		currentStrip.append((x,y,z))
...
"""
{
	garden {
		garden_low {
			laurel {
				team: {book: tetley, instruments: clewin},
				date: 2007-02-03,
				expedition: 'Votla Gora'
				strips { [ [
					{stn: 1, x: , y: z: }, ] [] 
				]
			}
...
"""
File is a file containing the .3d data.
output is an object with the following methods
move()
line()
"""
def readSurvey(file, output):
	...
	while True:
		...
		if code == '\x00': #STOP
			...
			output.move(0.01*x, 0.01*y, 0.01*z)
			...
			output.line(0.01*x, 0.01*y, 0.01*z)
...
if __name__=='__main__':
	...
	readSurvey(inputFile, surveyWriter)
	survey = surveyWriter.getSurvey()
	outputfile = sys.stdout
	if len(sys.argv)>2:
		outputfile = open(sys.argv[2], 'w')
	print >>outputfile, json.dumps(survey)
